...
def test_nodes():
	for x in city_map:
		for y in city_map[x]:
			assert y[0] in city_map, 'City not found; check spelling %r %r' %x %y[0]
			assert (x, y[1]) in city_map[y[0]], 'City is missing bidirectional path %r %r' %x %y[0] 
#			print(x, ': ', y, '\n\t', y[0], ': ', y[1])
		in_color = False
...
def get_city(x):
	if x in city_map:
		print(x, ':')
		for y in city_map[x]:
			print('\tIt costs ', y[1], ' to connect to ', y[0])
